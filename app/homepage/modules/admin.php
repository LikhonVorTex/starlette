...
if(strpos($message, "/subscribe") === 0 and $userId == $config['adminID']){
    $args = explode(" ", substr($message, 11));
    $userID = $args[0];
    $timer = $args[1];

        if(stripos($timer,'d')){
            $alreadyPrem = get_prem_info($userID);
            if($alreadyPrem > time()){
                $timerunix = add_days($alreadyPrem,str_replace('d','',$timer));
            }else{
                $timerunix = add_days(time(),str_replace('d','',$timer));
            }
    ...
    bot('sendmessage',[
        'chat_id'=>$chat_id,
        'text'=>regUserAsPremium($userID,$timerunix,$timer),
        'parse_mode'=>'html',
        'reply_to_message_id'=> $message_id,
    ]);
